...
void pipeMaster()
{
  ...
  printf("Total pipeCounter %d\n",pipeCounter);

  /* sets up the pipes */
  for (pCounter = 0; pCounter < pipeCounter; pCounter++) 
  {
    	pipe(fd+pCounter*2);
  }
  ...
  /* */
  for (pCounter = 0; pCounter < pipeCounter+1; pCounter++) {
    

    aID[pCounter] = fork();
	printf("creating child loop pCounter=%d\n", pCounter); 

    /* exit if failed */
    if (aID[pCounter] < 0) {
      fprintf(stderr, "%d fork failed", pCounter);
      exit(9);
    }
    /* Algorithm:
          command 0 | command 1 | command 2 | ... | command n
      =>  cmd0 [1,0] cmd1 [3,2] cmd2 [5,4] --> [n-2,n-1] cmd_n 
      To handle 1st command:
       cmd0 writes to 1.
       cmd1 read from 0, write to 3
       cmd2 read from 2, write to 5
       cmd3 read from 4, write to 6
       Rules:
       At node n, n process will READD from "(n-1)* 2 pipe
                                 WRITE to    (n*2) + 1
    */

    /* if not failed - handle child process */
    if (aID[pCounter] == 0) {
    	printf("pCounter %d\n",pCounter);

      /* First write from cmd1 */
      if (pCounter == 0) {
        printf("pCounter %d writePipeID 0 \n",pCounter);

        dup2(fd[1], STDOUT_FILENO);
        printf("completed dup: *pCounter %d writePipeID 0 \n",pCounter);

        Close_All_Pipes(fd);
        if(execvp(*newargv,newargv) < 0){
          fprintf(stderr, "ERROR execute child %d failed\n",pCounter);
          exit(9);
        }
      }
      /* handle last command - read but not write.*/
      else if (pCounter == pipeCounter)
      {
        int readPipeID = (pCounter - 1) * 2;
        printf("pCounter %d readPipeID %d \n",pCounter,readPipeID);
 
        dup2(fd[readPipeID], STDIN_FILENO);
        printf("completed pCounter %d readPipeID %d \n",pCounter,readPipeID);

        Close_All_Pipes(fd);
        if(execvp(newargv[(pipeArr[pCounter-1])], newargv+(*(pipeArr+pCounter-1))) < 0){
          fprintf(stderr, "ERROR execute child %d failed\n",pCounter);
          exit(9);
        }
      }
      /* handle all command in between */
      else if(pCounter > 0)
      {

        int readPipeID = (pCounter - 1) * 2;
        int writePipeID = pCounter * 2 + 1; 

        printf("pCounter %d readPipeID %d writePipeID %d \n",pCounter,readPipeID,writePipeID);
        dup2(fd[readPipeID], STDIN_FILENO);
        Close_All_Pipes(fd);

        dup2(fd[writePipeID], STDOUT_FILENO);
        Close_All_Pipes(fd);
		if(execvp(newargv[(pipeArr[pCounter-1])], newargv+(*(pipeArr+pCounter-1))) < 0){
			fprintf(stderr, "ERROR execute child %d failed\n",pCounter);
			exit(9);
